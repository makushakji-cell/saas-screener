...
function status() {
  ...
  const track = s.days
    ...
    : '';
  $('status').textContent =
    `${cnt('BUY')} buy · ${cnt('HOLD')} hold · ${cnt('SELL')} sell · updated ${ago(q)}${track}`;
}
...
async function load(url = '/api/screener') {
  try {
    ...
    $('error').innerHTML = `<div class="err">Could not reach the screener.
      Is the server running? <code>python -m saasscreener.server</code></div>`;
  }
}
